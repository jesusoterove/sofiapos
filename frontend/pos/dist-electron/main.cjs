...
var updateServerUrl = process.env.UPDATE_SERVER_URL || process.env.VITE_UPDATE_SERVER_URL || "https://updates.sofiapos.com";
if (process.platform === "win32") {
...
console.log(`[AutoUpdater] Current version: ${import_electron.app.getVersion()}`);
...
import_electron.ipcMain.handle("install-update", async () => {
  try {
    console.log("[IPC] Installing update and restarting...");
    import_electron_updater.autoUpdater.quitAndInstall(false, false);
    return { success: true };
  } catch (error) {
